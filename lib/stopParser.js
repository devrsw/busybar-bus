...
function parseStopInfo(html) {
  ...
  const departures = [...text.matchAll(/(\d+\s*mins|\d{1,2}:\d{2}\s*[AP]M)\s*(\d+)\s+([A-Za-z0-9 .'-]+?)(?=\s*(?:\d+\s*mins|\d{1,2}:\d{2}\s*[AP]M|Real-time Departure|$))/gi)];
